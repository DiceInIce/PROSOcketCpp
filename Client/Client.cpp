...
int main() {
	...
	try {
		...
		while (true) {
			...
			cout << "5. Изменения существующего товара по ID" << endl;
			cout << "6. Выход" << endl;
			...
			if (choice == 1) {
				...
				request = "ADD_MANUAL " + p.to_string_custom();
			}
			else if (choice == 4) {
				...
				Product p = Product::input_product(true);
				request = "UPDATE " + p.to_string_custom();
			}
			else if (choice == 6) {
				request = "EXIT";
			}
			...
			else if (response.rfind("DONE|", 0) == 0) {
				cout << "\nОперация прошла успешно" << endl;
			}
